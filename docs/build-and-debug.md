...
### Output files
```
build/stepper.elf   — ELF with debug symbols (for GDB)
build/stepper.hex   — Intel HEX (for flashing)
build/stepper.bin   — raw binary
build/stepper.map   — linker map
```

### Expected size (lookup table build, -O0)
```
text: ~70KB   data: ~748B   bss: ~14KB
```
...
### GDB Setup
The `.gdbinit` in `firmware/` is loaded automatically. It contains:
...
> **Note**: PyCortexMDebug is cloned inside `firmware/PyCortexMDebug/` and loaded
...
(gdb) ld       ← flash stepper.hex, verify sections
...
```
                       ...
                       +-----+
         (front row)   |     |   (back row)
 pin 1   B12  ●───────┤     ├───────●  B12  ← WRONG, back row starts pin 40
                                              See diagram below.
